  ...
  try {
    ...
    if (body.isRevision === true) {
      ...
      const pdfProps = {
        ...
        includeCategoryName: body.includeCategoryName !== undefined ? !!body.includeCategoryName : (existingQuotation.includeCategoryName ?? true),
        includeSectionHeadings: body.includeSectionHeadings !== undefined ? !!body.includeSectionHeadings : (existingQuotation.includeSectionHeadings ?? true),
      ...
      const updatedQuotation = await prisma.$transaction(async (tx) => {
        ...
        return await tx.quotation.create({
          data: {
            ...
            sharepointUrl,
            notes: body.revisionNotes || notes || existingQuotation.notes || null,
            ...
            commonRemark: body.commonRemark !== undefined ? (body.commonRemark || null) : (existingQuotation.commonRemark || null),
            commonRemarkHighlight: body.commonRemarkHighlight !== undefined ? !!body.commonRemarkHighlight : (existingQuotation.commonRemarkHighlight ?? false),
            commonRemarkStyle: body.commonRemarkStyle || existingQuotation.commonRemarkStyle || "AMBER",
    ...
    if (body.isUpdate === true) {
      ...
      const rawUpdateItems = await Promise.all(items.map(async (item: any, idx: number) => {
        ...
        return {
          ...
          estimatorNotes: item.estimatorNotes || null,
          estimatorId: item.estimatorId || null,
          costingRequestedAt: item.costingRequestedAt ? new Date(item.costingRequestedAt) : null,
          costingCompletedAt: item.costingCompletedAt ? new Date(item.costingCompletedAt) : null,
        }
      }))
      ...
      const updatedQuotation = await prisma.$transaction(async (tx) => {
        ...
        return await tx.quotation.update({
          ...
          data: {
            ...
            includeSalesAgent: !!includeSalesAgent,
            includeCompanySeal: includeCompanySeal ?? false,
            includeCategoryName: body.includeCategoryName !== undefined ? !!body.includeCategoryName : true,
            includeSectionHeadings: body.includeSectionHeadings !== undefined ? !!body.includeSectionHeadings : true,
            includeMaterialsFinishes: !!includeMaterialsFinishes,
            selectedMaterials: Array.isArray(selectedMaterials) ? (selectedMaterials as any) : [],
            salesAgentId: includeSalesAgent ? (salesAgentId || null) : null,
            salesAgentName: includeSalesAgent ? (salesAgentName || null) : null,
