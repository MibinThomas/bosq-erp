...
export async function POST(request: Request) {
  try {
    ...
    const rawItems = await Promise.all(items.map(async (item: any, idx: number) => {
    ...
    const quotationItemsToCreate = rawItems
    ...
    const pdfProps = {
      ...
      preparedByEmail: creatorUser.email || null,
      ...
      includeSectionHeadings: body.includeSectionHeadings ?? true,
